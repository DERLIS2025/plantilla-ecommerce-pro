...
export const productPriceTierSchema = z
  .object({
    minQuantity: z.coerce
      .number()
      .positive('La cantidad desde debe ser mayor a cero.'),
  ...
  .refine(
    ...
    {
      message:
        'La cantidad hasta no puede ser menor que la cantidad desde.',
      path: ['maxQuantity']
    }
  );
...
export const productFormSchema = z
  .object({
    ...
    canonicalUrl: z
      .union([
        z.literal(''),
        z.string().url('La URL canonical no es válida.')
      ])
      .default('')
  })
    ...
    product.specifications.forEach((specification, index) => {
      ...
      if (hasKey !== hasValue) {
        context.addIssue({
          ...
          message:
            'Completá tanto el nombre como el valor de la especificación.'
        });
      }
    });
...
export type ProductImageDraft = z.infer<
